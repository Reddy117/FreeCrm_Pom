...
import com.freecrm.util.XlsReader;
...
public class BaseClass extends OR{
	...
	public static void selectDropLooklike(WebElement dropdown,WebElement box,String value,String tag) {
		try {
			...
			dropdown.click();
			List<WebElement> items=box.findElements(By.tagName(tag));
			for(WebElement item:items) {
				System.out.println(item.getText());
				if(item.getText().equals(value)) {
					item.click();
